#modifying code from open source game
# adding alot more
#need to make the meteors fall slower +
#need to cut invisible layer *
#add lasers *
#fix music when you die it doesnt reset *
#NEED TO SLOW DOWN ASTEROIDS +
...
# Set Caption
pygame.display.set_caption("Asteroids V1 by luca " + str(version))
...
main_font = "game_data/BNR.ttf"

pause = False

# can change just check properties of music (SICKAGE!!!)
if mute_txt == "False":
    pygame.mixer.music.load("game_data/good.mp3")
...
def main_menu():
    ...
    while True:
        ...
        # title onto screen
        text = pygame.font.Font(main_font, 100)
        textsurf, textrect = text_objects("Asteroids Version 1 (may contain bugs)", text)
        textrect.center = ((res_w / 2), 110)
        screen.blit(textsurf, textrect)

        # Draws "by luca (and help from starter code)" to the Screen
        text = pygame.font.Font(main_font, 50)
        textsurf, textrect = text_objects("by IILucaII", text)
        textrect.center = ((res_w / 2), 180)
        screen.blit(textsurf, textrect)

        # space to start and all options
        text = pygame.font.Font(main_font, 50)
        textsurf, textrect = text_objects("Press [SPACEBAR] to Start playing .... ENTER IF YOU DARE!!!!!", text)
        textrect.center = ((res_w / 2), (res_h / 2))
        screen.blit(textsurf, textrect)
        ...
        text = pygame.font.Font(main_font, 30)
        textsurf, textrect = text_objects_y("Move WASD", text)
        ...
        text = pygame.font.Font(main_font, 30)
        textsurf, textrect = rg_fs_("Audio (ON/OFF): [M]", text)
        ...
        text = pygame.font.Font(main_font, 50)
        textsurf, textrect = text_objects_r("MULTIPLAYER COMING SOON (AND LASERS)", text)
        textrect.center = ((res_w / 2), (res_h / 2 + 400))
        screen.blit(textsurf, textrect)

        text = pygame.font.Font(main_font, 20)
        textsurf, textrect = text_objects_r("Thank dad for that idea ^", text)
        textrect.center = ((res_w / 2), (res_h / 2 + 420))
        screen.blit(textsurf, textrect)

        # Draws "Version: X" to the Screen
        text = pygame.font.Font(main_font, 30)
        textsurf, textrect = text_objects("V" + str(version), text)
        textrect.center = ((res_w / 2), 15)
        screen.blit(textsurf, textrect)
...
def game_loop():
    ...
    change_x = 0
    change_y = 0
#here YOU CHANGE SPEED IMPORTANT!!! NEEDED LATER ON!!!!!!!!!!!!!!!!!!!!!!
    # Color Generator
    ...
    # Music
    if mute_txt == "False":
        pygame.mixer.music.load("game_data/bombscare.mp3")
        pygame.mixer.music.play(-1)
    elif mute_txt == "True":
        pass
